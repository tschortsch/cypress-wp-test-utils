Cypress.Commands.add( 'ensureSidebarOpened', () => {
	cy
		.window({ log: false }).then( ( window ) => {
			...
			cy
				.get( '.edit-post-header__settings [aria-label="Settings"]', { log: false } ).click( { force: true, log: false } )
				.then(() => {
					if ( ! window.document.querySelector( '.edit-post-sidebar' ) ) {
						cy.get( '.edit-post-header__settings [aria-label="Settings"]', { log: false } ).click( { force: true, log: false } )
